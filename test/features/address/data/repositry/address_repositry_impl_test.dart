...
void main() {
  ...
  setUp(() {
    ...
    provideDummy<LocalDsResult<List<CountryEntity>>>(
      LocalDsFailedResult("Dummy Error"),
    );
  ...
  group("Address Repositry", () {
    ...
    group("Add Address Repositery", () {
      test("return ApiSuccessResult when data source success", () async {
        final addresses = successResponse.address!
            .map((e) => e.toEntity())
            .toList();
        when(
          mockAddressRemoteDataSource.addAddress(request, token),
      ...
      test("return ApiFailedResult when data source failed", () async {
        ...
        when(
          mockAddressRemoteDataSource.addAddress(any, token),
        ).thenAnswer((_) async => FailedResult(errorMessage));
        final result = await addressRepositry.addAddress(request, token);
        expect(result, isA<FailedResult>());
        expect((result as FailedResult).errorMessage, errorMessage);
        verify(
          mockAddressRemoteDataSource.addAddress(request, token),
        ).called(1);
      });
    ...
    group("Update Address Repositry", () {
      test("return ApiSuccessResult when datasource success", () async {
        final addresses = successResponse.address!
            ...
            .toList();
        when(
          mockAddressRemoteDataSource.updateAddress(token, id, request),
        ).thenAnswer((_) async => SucessResult(addresses));
    ...
    group("Remove Address Repositry", () {
      test("return ApiSuccessResult when datasource success", () async {
        ...
        when(
          mockAddressRemoteDataSource.removeAddress(token, id),
        ).thenAnswer((_) async => SucessResult(success));
        final result = await addressRepositry.removeAddress(token, id);
        expect(result, isA<SucessResult<RemoveAddressDto>>());
        expect((result as SucessResult).sucessResult, success);
    ...
    group("Get Address Repositry", () {
      test("return ApiSuccessResult when datasource success", () async {
        ...
        when(
          mockAddressRemoteDataSource.getAllAddress(token),
        ).thenAnswer((_) async => SucessResult(addresses));
    ...
    group("Get Governate ", () {
      test("return list of governorates when data source success", () async {
        ...
        when(
          mockAddressRemoteDataSource.getGovernorates(),
    ...
    group("Get Countries", () {
      test("return list of countries when data source success", () async {
        ...
        when(
          mockAddressRemoteDataSource.getCountries(),
    ...
    group("Get States", () {
      test("return list of states when datasource success", () async {
        ...
        when(
          mockAddressRemoteDataSource.getStates(governateId),
