...
@GenerateMocks([
  GetAddAddressUseCase,
  GetAllAddressesUseCase,

  GetAllStatesUseCase,
  GetAllCountriesUseCase,
  GetAllGovernorateUseCase,
...
void main() {
  ...
  setUp(() async {
    ...
    provideDummy<Result<RemoveAddressDto>>(FailedResult("Dummy Error"));
    provideDummy<LocalDsResult<List<CountryEntity>>>(
      LocalDsFailedResult("Dummy Error Countries"),
    );
    provideDummy<LocalDsResult<List<GovernorateEntity>>>(
      LocalDsFailedResult("Dummy Error Governorates"),
    );
    provideDummy<LocalDsResult<List<StateEntity>>>(
      LocalDsFailedResult("Dummy Error States"),
    );
  });
  ...
  final updatedLoginModel = LoginModel(user: updatedUser, token: token);
  group("Add Address", () {
    blocTest<AddressBloc, AddressState>(
      "emits [loading, success] when GetAddAddressEvent succeeds",
      build: () {
        when(
          mockAddAddressUseCase.addAddress(request, token),
        ).thenAnswer((_) async => SucessResult(address1));

        return bloc;
      },
      act: (bloc) =>
          bloc..add(GetAddAddressEvent(request: request, token: token)),
      expect: () => [
      ...
      verify: (_) =>
          verify(mockAddAddressUseCase.addAddress(request, token)).called(1),
    );
  });
  final error = "Failed to add address";
  blocTest(
    "emits [loading, failure] when GetAddAddressEvent succeeds",
    build: () {
      when(
        mockAddAddressUseCase.addAddress(request, token),
  ...
  group("Get Delete Addresses", () {
    ...
    blocTest(
      "emits [loading, success] when DeleteAddAddressEvent succeeds",
      build: () {
        when(
          mockDeleteAddressUseCase.removeAddress(token, id),
        ).thenAnswer((_) async => SucessResult(success));
        return bloc;
      },
      act: (b) => b..add(DeleteAddressEvent(id: id, token: token)),
      expect: () => [
        AddressState(deleteAddressRequestState: RequestState.loading),
        AddressState(
      ...
      verify: (_) =>
          verify(mockDeleteAddressUseCase.removeAddress(token, id)).called(1),
    );
    final error = "Failed to delete address";
    blocTest(
      "emits [loading, failure] when DeleteAddAddressEvent failed",
      build: () {
        when(
          mockDeleteAddressUseCase.removeAddress(token, id),
        ).thenAnswer((_) async => FailedResult(error));
    ...
    group("Update Adress", () {
      ...
      blocTest(
        "emits [loading, success] when UpdateAddAddressEvent succeeds",
        build: () {
          when(
            mockUpdateAddressUseCase.updateAddress(token, id, request),
          ).thenAnswer((_) async => SucessResult(address));

          return bloc;
        },
        act: (b) =>
            b..add(UpdateAddressEvent(id: id, token: token, request: request)),
        expect: () => [
    ...
    blocTest(
      "emits [loading, failure] when UpdateAddAddressEvent failed",
      build: () {
        when(
          mockUpdateAddressUseCase.updateAddress(token, id, request),
    ...
    group("Get All Addresses", () {
      ...
      blocTest(
        ...
        build: () {
          when(
            mockGetAllAddressUseCase.getAllAddress(token),
          ).thenAnswer((_) async => SucessResult(addresses));
          return bloc;
        },
        act: (b) => b..add(GetAllddressEvent(token: token)),
        expect: () => [
          AddressState(getAddressRequestState: RequestState.loading),
          AddressState(
        ...
        verify: (_) =>
            verify(mockGetAllAddressUseCase.getAllAddress(token)).called(1),
      );
    });
    final errorMsg = "Failed to load Addresses";
    blocTest(
      "emits [loading, failure] when GetAddAddressEvent failed",
      build: () {
        when(
          mockGetAllAddressUseCase.getAllAddress(token),
        ).thenAnswer((_) async => FailedResult(errorMsg));
        return bloc;
      },
      act: (b) => b..add(GetAllddressEvent(token: token)),
      expect: () => [
  ...
  group("GET Countries Event", () {
    blocTest<AddressBloc, AddressState>(
      "emits loading and success states when getCountries returns success",
      build: () {
        when(
          mockGetCountriesUseCase.getCountries(),
        ).thenAnswer((_) async => LocalDsSucessResult(countries));
        return bloc;
      },
    ...
    blocTest<AddressBloc, AddressState>(
      ...
      build: () {
        when(
          mockGetCountriesUseCase.getCountries(),
        ).thenAnswer((_) async => LocalDsFailedResult(countriesError));
  ...
  group("Get Governorates Event", () {
    blocTest<AddressBloc, AddressState>(
      "emits loading and success states when getGovernorates returns success",
      build: () {
        when(
          mockGetGovernorateUseCase.getGovernorates(),
        ).thenAnswer((_) async => LocalDsSucessResult(governorates));
        return bloc;
      },
      act: (bloc) => bloc..add(GetGovernorateEvent()),
      expect: () => [
        AddressState(governorateRequestState: RequestState.loading),
        AddressState(
      ...
      verify: (_) =>
          verify(mockGetGovernorateUseCase.getGovernorates()).called(1),
    );
    blocTest<AddressBloc, AddressState>(
      "emits loading and failure states when getGovernorates returns failed",
      build: () {
        when(mockGetGovernorateUseCase.getGovernorates()).thenAnswer(
          (_) async => LocalDsFailedResult(governoratesErrorMessage),
        );
        return bloc;
      },
      act: (bloc) => bloc.add(GetGovernorateEvent()),
  ...
  group("Get States Event", () {
    blocTest<AddressBloc, AddressState>(
      ...
      build: () {
        when(
          mockGetStatesUseCase.getStates(governateId),
        ).thenAnswer((_) async => LocalDsSucessResult(states));
        return bloc;
      },
      ...
      verify: (_) =>
          verify(mockGetStatesUseCase.getStates(governateId)).called(1),
    );
    blocTest(
      "emits loading and failed states when getStates returns success",
      build: () {
        when(
          mockGetStatesUseCase.getStates(governateId),
        ).thenAnswer((_) async => LocalDsFailedResult(statesError));
        return bloc;
      },
      act: (bloc) => bloc..add(GetStatesEvent(governateId: governateId)),
      expect: () => [
