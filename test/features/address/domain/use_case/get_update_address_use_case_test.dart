...
void main() {
  ...
  test('should return ApisuccessResult when repo success', ()async {
    ...
    when(mockAddressRepositry.updateAddress(token,id, request)).thenAnswer((_)
    async=>ApiSucessResult(addresses));
    final result=await getUpdateAddressUseCase.updateAddress(token, id, request);
  ...
  test('should throw an Exception when repository throws an exception', () async {
    ...
    when(mockAddressRepositry.updateAddress(token, id, request)).thenThrow(exception);
