...
// Fake Cart Response
  ...
  group("test addProductToCart function", () {
    test(
      ...
      when(mockCartApiService.addProductToCart(
      ...
      final res =
          await cartRemoteDataSouceImp.addProductToCart(cartItemRequestEntity);
      ...
      expect(acResult.sucessResult.cart!.cartItems!.length,
          fakeCart.cartItems!.length);
      expect(acResult.sucessResult.cart!.cartItems![0].id,
          fakeCart.cartItems![0].id);

      expect(acResult.sucessResult.cart!.cartItems![0].product!.id,
          fakeCart.cartItems![0].product!.id);

      verify(mockCartApiService.addProductToCart(
              CartItemRequestModel.toModel(cartItemRequestEntity)))
          .called(1);
    });

    test(
        "when call addProductToCart with CartItemRequestEntity as a wrong parameter it should return ApiFailedResult with dio exception message  ",
        () async {
      final String dioException = "dio Exception";
      final DioException mockDioException = DioException(
          requestOptions: RequestOptions(path: ''), message: dioException);
      // arrange
      CartItemRequestEntity cartItemRequestEntity = CartItemRequestEntity(
        product: "12345",
        quantity: 3,
    ...
    test(
        "when call addProductToCart with CartItemRequestEntity as a wrong parameter it should return ApiFailedResult with exception message  ",
        () async {
  ...
  group("test getCartOfUser function", () {
    test("when call getCartOfUser  it should return ApiSucessResult  ",
        () async {
      // arrange

      when(mockCartApiService.getCartUser())
          .thenAnswer((_) async => fakeCartResponse);
    ...
    test(
        "when call getCartOfUser  it should return ApiFailedResult with dio exception message  ",
        () async {
      final String dioException = "dio Exception";
      final DioException mockDioException = DioException(
          requestOptions: RequestOptions(path: ''), message: dioException);
      ...
      final res =
          await cartRemoteDataSouceImp.getCartOfUser();
 ...
 group("test deleteItemFromCart function", () {
  ...
  test("should return ApiSucessResult when delete is successful", () async {
    // arrange
    when(mockCartApiService.deleteItemFromCart(itemId))
        .thenAnswer((_) async => fakeCartResponse);
  ...
  test("should return ApiFailedResult when DioException occurs", () async {
  ...
  test("should return ApiFailedResult when Exception occurs", () async {
...
group("test updateQuantity function", () {
  ...
  test("should return ApiSucessResult when update is successful", () async {
    // arrange
    when(mockCartApiService.updateQuantity(
            updatedReq.itemId,
    ...
    final acResult = res as ApiSucessResult<CartResponseEntity>;
    expect(acResult.sucessResult.cart!.id, fakeCart.id);
    verify(mockCartApiService.updateQuantity(
            updatedReq.itemId,
            UpdatedRequestBodyModel.toModel(updatedReq.updatedRequestBody)))
        .called(1);
  });
