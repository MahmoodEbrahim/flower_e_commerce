...
final result=await getChangePasswordUseCase.changePassword(request, token);
...
test("return ApiFailedResult when  ProfileRepository fails", ()async{
 ...
 when(mockProfileRepository.changePassword(request, token)).thenAnswer((_)async=>
 ...
 verify(mockProfileRepository.changePassword(request, token)).called(1);
  ...
  test("return ApiFailedResult when  ProfileRepository fails", ()async{
   ...
   when(mockProfileRepository.changePassword(request, token)).thenAnswer((_)async=>ApiFailedResult(ServerFailure.fromDioError(dioException).errorMessage));
