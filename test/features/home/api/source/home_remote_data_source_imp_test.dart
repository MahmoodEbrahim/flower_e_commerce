...
void main() {
  ...
  group("test searchProducts in homeRemoteDataSourceImp", () {
    test(
      ...
      () async {
        final fakeDtoList = CategoryProductsResponseDto(
          products: fakeProductList,
        );
        final fakeEntityList = fakeProductList
            .map((e) => e.toEntity())
            .toList();
        ...
        expect((result as SucessResult).sucessResult, equals(fakeEntityList));
    ...
    test(
      'should return FailedResult when general Exception is thrown',
      () async {
        when(
          mockHomeApiService.searchProducts("rose"),
  ...
  group("test getProductsByCategoryId in homeRemoteDataSourceImp", () {
    test("should return successApiResult", () async {
      when(
        mockHomeApiService.getProductsByCategoryId(catId),
    ...
    test("should return failedApiResult with dio exception", () async {
      ...
      final acResult = res as FailedResult<List<ProductsEntity>>;
      expect(acResult.errorMessage, equals(dioMessage));
    });

    test("should return failedApiResult with exception", () async {
  ...
  group("test getSearchProducts in homeRemoteDataSourceImp", () {
    test("should return successApiResult when category != null", () async {
      String filter = "price";

      when(
        mockHomeApiService.getSearchProducts(filter, catId),
      ).thenAnswer((_) async => searchResponseModel);

      final res = await homeRemoteDataSourceImp.getSearchProducts(
        filter,
        catId,
      );
  ...
  group("test getHomeData in homeRemoteDataSourceImp", () {
    test("should return ApiSuccessResult when API returns HomeModel", () async {
      // arrange
      ...
      when(mockHomeApiService.getHomeData()).thenAnswer((_) async => homeModel);
    ...
    test("should return FailedResult when Exception is thrown", () async {
      when(
        mockHomeApiService.getHomeData(),
      ).thenThrow(Exception("Unexpected error"));
