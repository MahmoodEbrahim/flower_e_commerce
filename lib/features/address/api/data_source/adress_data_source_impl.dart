...
class AddressRemoteDataSourceImpl implements AddressRemoteDataSource {
  ...
  Future<Result<List<AddressEntity>>> updateAddress(
    ...
    try {
      final response = await _apiServices.updateAddress(
        "Bearer $token",
        id,
        request,
      );
  ...
  Future<LocalDsResult<List<GovernorateEntity>>> getGovernorates() async {
    try {
      ...
      final governates = governatesData
          .map(
            (json) => GovernorateEntity(
              id: json["id"],
  ...
  Future<LocalDsResult<List<StateEntity>>> getStates(String governateId) async {
    try {
      ...
      final statesByCities = states
          .where((e) => e.governorateId == governateId)
          .toList();
